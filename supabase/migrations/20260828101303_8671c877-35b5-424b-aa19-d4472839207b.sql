-- LP-C.2.2: server-derived duration enforcement for Field Note media.

-- 1. Members may no longer finalize attachments directly (that path trusted the
--    client-declared duration). Finalization now happens only through the
--    verified server routine below.
REVOKE EXECUTE ON FUNCTION public.living_media_finalize(uuid) FROM authenticated;

-- 2. Verified finalization: duration is supplied by the server after probing the
--    actual uploaded bytes. Anything over the accepted limits is discarded.
