...
BEGIN
  ...
  -- Bypass FK & trigger constraints to auth.users / immutability for this transactional in-memory test.
  ...
  INSERT INTO public.manual_full_access_grants(user_id, starts_at, expires_at, granted_by, revoked_at)
    VALUES (v_revoked, v_now - interval '10 days', v_now + interval '10 days', v_admin, v_now - interval '1 hour');

  INSERT INTO public.manual_access_legacy_bucket_history(user_id, bucket_key, starts_at, ends_at, original_grant_id, original_created_at)
    VALUES
      (v_legacy_before, 'devotion',    v_now + interval '5 days',  v_now + interval '20 days', gen_random_uuid(), v_now),
      (v_legacy_during, 'remembrance', v_now - interval '5 days',  v_now + interval '20 days', gen_random_uuid(), v_now),
      (v_legacy_after,  'communion',   v_now - interval '30 days', v_now - interval '5 days',  gen_random_uuid(), v_now);
