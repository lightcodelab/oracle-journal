...
BEGIN
  ...
  PERFORM public.stripe_webhook_record_env_mismatch(
    '__p32_env_mismatch__'::text, 'live'::text, true, 'livemode_true'::text);
  ...
  v_offer := public.admin_test_get_membership_offer_at('2026-09-13T23:59:59Z'::timestamptz, 'test');
  RETURN QUERY SELECT 'founding-boundary-before-open'::text, (v_offer->>'is_founding_window')::boolean=false, v_offer;
  v_offer := public.admin_test_get_membership_offer_at('2026-09-14T00:00:00Z'::timestamptz, 'test');
  RETURN QUERY SELECT 'founding-boundary-open-instant'::text, (v_offer->>'is_founding_window')::boolean=true, v_offer;
  v_offer := public.admin_test_get_membership_offer_at('2026-09-29T00:00:00Z'::timestamptz, 'test');
  RETURN QUERY SELECT 'founding-boundary-mid-window'::text, (v_offer->>'is_founding_window')::boolean=true, v_offer;
  v_offer := public.admin_test_get_membership_offer_at('2026-12-13T22:59:59Z'::timestamptz, 'test');
  RETURN QUERY SELECT 'founding-boundary-before-close'::text, (v_offer->>'is_founding_window')::boolean=true, v_offer;
  v_offer := public.admin_test_get_membership_offer_at('2026-12-13T23:00:00Z'::timestamptz, 'test');
  RETURN QUERY SELECT 'founding-boundary-close-instant'::text, (v_offer->>'is_founding_window')::boolean=false, v_offer;
  v_offer := public.admin_test_get_membership_offer_at('2026-12-13T23:00:01Z'::timestamptz, 'test');
  ...
  RETURN QUERY SELECT 'test-founder-does-not-flip-live-badge'::text,
    COALESCE((public.get_member_state(v_uid2)->>'founder_badge')::boolean, false) = false,
    public.get_member_state(v_uid2);
