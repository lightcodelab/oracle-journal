DROP FUNCTION IF EXISTS public.search_temple(text);

CREATE OR REPLACE FUNCTION public.search_temple(_q text)
RETURNS TABLE(kind text, id uuid, title text, subtitle text, deck_id uuid, deck_name text, card_number integer, image_url text, door text, tags text[], rank real, score real)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  WITH q AS (
    SELECT btrim(coalesce(_q, '')) AS raw
  ),
  words AS (
    SELECT q.raw,
           (SELECT array_agg(DISTINCT s.w)
              FROM (SELECT unnest(regexp_split_to_array(lower(q.raw), '[^a-z0-9]+')) AS w) s
             WHERE length(s.w) >= 3
               AND s.w NOT IN ('the','and','for','with','that','this','was','are','you','your','yours','from','have','has','had','feel','feels','feeling','felt','but','not','all','any','when','what','why','how','can','get','got','just','like','really','very','been','being','about','into','out','over','keep','keeps','kept','always','still','they','them','their','she','her','who','because','than','then','there','here','some','much','more','most','also','only','even','ever','never','make','makes','made','want','wants','wanted','need','needs','needed')
           ) AS ws
      FROM q
  ),
  pats AS (
    SELECT w.raw,
           COALESCE(w.ws, ARRAY[]::text[]) AS ws,
           (SELECT array_agg('\m' || u.x)
              FROM (SELECT unnest(COALESCE(w.ws, ARRAY[]::text[])) AS x) u
             WHERE length(u.x) >= 2) AS wp,
           CASE WHEN COALESCE(array_length(w.ws, 1), 0) > 0
                THEN w.ws ELSE ARRAY[lower(w.raw)] END AS fz,
           '\\m' || regexp_replace(lower(w.raw), '([^a-z0-9 ])', '\\\1', 'g') || '\\M' AS rp
      FROM words w
  )
  SELECT * FROM (
    -- exact phrase / word-prefix / tag matches on any term
    -- rank: 1 = title match, 2 = tag match, 3 = body/content match, 4 = fuzzy
    SELECT 'deck'::text AS kind, d.id, d.name AS title, d.description AS subtitle,
           d.id AS deck_id, d.name AS deck_name, NULL::integer AS card_number,
           d.thumbnail_url AS image_url, 'remembrance'::text AS door,
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                      WHERE dta.deck_id = d.id), '{}') AS tags,
           (CASE
              WHEN d.name ~* p.rp OR (p.wp IS NOT NULL AND d.name ~* ANY (p.wp)) THEN 1
              WHEN p.wp IS NOT NULL AND EXISTS (SELECT 1 FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                       WHERE dta.deck_id = d.id AND t.name ~* ANY (p.wp)) THEN 2
              ELSE 3
            END)::real AS rank,
           100.0::real AS score
      FROM (SELECT * FROM decks WHERE is_published = true OR public.has_role(auth.uid(), 'admin'::public.app_role)) d, pats p
     WHERE length(p.raw) >= 2 AND (p.wp IS NOT NULL OR length(p.raw) >= 2)
       AND (d.name ~* p.rp
         OR coalesce(d.description, '') ~* p.rp
         OR (p.wp IS NOT NULL AND (
              d.name ~* ANY (p.wp)
           OR coalesce(d.description, '') ~* ANY (p.wp)
           OR coalesce(d.theme, '') ~* ANY (p.wp)
           OR EXISTS (SELECT 1 FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                       WHERE dta.deck_id = d.id AND t.name ~* ANY (p.wp)))))
     UNION ALL
    SELECT 'card'::text, c.id,
           COALESCE(NULLIF(btrim(c.card_title), ''), 'Card ' || c.card_number),
           left(regexp_replace(coalesce(NULLIF(btrim(c.card_details), ''), ''), '\s+', ' ', 'g'), 200),
           c.deck_id, dk.name, c.card_number, dk.thumbnail_url, 'remembrance'::text,
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                      WHERE cta.card_id = c.id), '{}'),
           (CASE
              WHEN coalesce(c.card_title, '') ~* p.rp OR (p.wp IS NOT NULL AND coalesce(c.card_title, '') ~* ANY (p.wp)) THEN 1
              WHEN p.wp IS NOT NULL AND EXISTS (SELECT 1 FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                       WHERE cta.card_id = c.id AND t.name ~* ANY (p.wp)) THEN 2
              ELSE 3
            END)::real,
           90.0::real
      FROM cards c JOIN (SELECT * FROM decks WHERE is_published = true OR public.has_role(auth.uid(), 'admin'::public.app_role)) dk ON dk.id = c.deck_id, pats p
     WHERE length(p.raw) >= 2
       AND (coalesce(c.card_title, '') ~* p.rp
         OR coalesce(c.card_details, '') ~* p.rp
         OR (p.wp IS NOT NULL AND (
              coalesce(c.card_title, '') ~* ANY (p.wp)
           OR coalesce(c.card_details, '') ~* ANY (p.wp)
           OR coalesce(c.content_sections::text, '') ~* ANY (p.wp)
           OR EXISTS (SELECT 1 FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                       WHERE cta.card_id = c.id AND t.name ~* ANY (p.wp)))))
     UNION ALL
    SELECT 'course'::text, co.id, co.title, co.description,
           NULL::uuid, NULL::text, NULL::integer, co.image_url,
           COALESCE(cc.page, co.door_type),
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                      WHERE a.course_id = co.id), '{}'),
           (CASE
              WHEN co.title ~* p.rp OR (p.wp IS NOT NULL AND co.title ~* ANY (p.wp)) THEN 1
              WHEN p.wp IS NOT NULL AND EXISTS (SELECT 1 FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                       WHERE a.course_id = co.id AND t.name ~* ANY (p.wp)) THEN 2
              ELSE 3
            END)::real,
           80.0::real
      FROM courses co
      LEFT JOIN content_categories cc ON cc.id = co.location_id, pats p
     WHERE length(p.raw) >= 2 AND co.is_published
       AND (co.title ~* p.rp
         OR coalesce(co.description, '') ~* p.rp
         OR (p.wp IS NOT NULL AND (
              co.title ~* ANY (p.wp)
           OR coalesce(co.description, '') ~* ANY (p.wp)
           OR EXISTS (SELECT 1 FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                       WHERE a.course_id = co.id AND t.name ~* ANY (p.wp))
           OR EXISTS (SELECT 1 FROM lessons l WHERE l.course_id = co.id
                        AND (coalesce(l.title, '') ~* ANY (p.wp)
                          OR coalesce(l.description, '') ~* ANY (p.wp)
                          OR coalesce(l.content, '') ~* ANY (p.wp))))))
     UNION ALL
    -- fuzzy (typo tolerant) matches per term, excluding rows already matched above
    SELECT 'deck'::text, d.id, d.name, d.description, d.id, d.name, NULL::integer,
           d.thumbnail_url, 'remembrance'::text,
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                      WHERE dta.deck_id = d.id), '{}'),
           4.0::real,
           s.sc
      FROM (SELECT * FROM decks WHERE is_published = true OR public.has_role(auth.uid(), 'admin'::public.app_role)) d, pats p,
           LATERAL (SELECT max(greatest(
                      similarity(lower(d.name), w),
                      COALESCE((SELECT max(similarity(lower(t.name), w))
                                  FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                                 WHERE dta.deck_id = d.id), 0))) AS sc
                      FROM unnest(p.fz) w) s
     WHERE length(p.raw) >= 3 AND s.sc >= 0.30
       AND NOT (d.name ~* p.rp
             OR coalesce(d.description, '') ~* p.rp
             OR (p.wp IS NOT NULL AND (
                  d.name ~* ANY (p.wp)
               OR coalesce(d.description, '') ~* ANY (p.wp)
               OR coalesce(d.theme, '') ~* ANY (p.wp)
               OR EXISTS (SELECT 1 FROM deck_tag_assignments dta JOIN course_tags t ON t.id = dta.tag_id
                           WHERE dta.deck_id = d.id AND t.name ~* ANY (p.wp)))))
     UNION ALL
    SELECT 'card'::text, c.id,
           COALESCE(NULLIF(btrim(c.card_title), ''), 'Card ' || c.card_number),
           left(regexp_replace(coalesce(NULLIF(btrim(c.card_details), ''), ''), '\s+', ' ', 'g'), 200),
           c.deck_id, dk.name, c.card_number, dk.thumbnail_url, 'remembrance'::text,
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                      WHERE cta.card_id = c.id), '{}'),
           4.0::real,
           s.sc
      FROM cards c JOIN (SELECT * FROM decks WHERE is_published = true OR public.has_role(auth.uid(), 'admin'::public.app_role)) dk ON dk.id = c.deck_id, pats p,
           LATERAL (SELECT max(greatest(
                      similarity(lower(coalesce(c.card_title, '')), w),
                      COALESCE((SELECT max(similarity(lower(t.name), w))
                                  FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                                 WHERE cta.card_id = c.id), 0))) AS sc
                      FROM unnest(p.fz) w) s
     WHERE length(p.raw) >= 3 AND s.sc >= 0.30
       AND NOT (coalesce(c.card_title, '') ~* p.rp
             OR coalesce(c.card_details, '') ~* p.rp
             OR (p.wp IS NOT NULL AND (
                  coalesce(c.card_title, '') ~* ANY (p.wp)
               OR coalesce(c.card_details, '') ~* ANY (p.wp)
               OR coalesce(c.content_sections::text, '') ~* ANY (p.wp)
               OR EXISTS (SELECT 1 FROM card_tag_assignments cta JOIN course_tags t ON t.id = cta.tag_id
                           WHERE cta.card_id = c.id AND t.name ~* ANY (p.wp)))))
     UNION ALL
    SELECT 'course'::text, co.id, co.title, co.description, NULL::uuid, NULL::text,
           NULL::integer, co.image_url, COALESCE(cc.page, co.door_type),
           COALESCE((SELECT array_agg(t.name ORDER BY t.name)
                       FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                      WHERE a.course_id = co.id), '{}'),
           4.0::real,
           s.sc
      FROM courses co
      LEFT JOIN content_categories cc ON cc.id = co.location_id, pats p,
           LATERAL (SELECT max(greatest(
                      similarity(lower(co.title), w),
                      COALESCE((SELECT max(similarity(lower(t.name), w))
                                  FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                                 WHERE a.course_id = co.id), 0),
                      COALESCE((SELECT max(similarity(lower(coalesce(l.title, '')), w))
                                  FROM lessons l WHERE l.course_id = co.id), 0))) AS sc
                      FROM unnest(p.fz) w) s
     WHERE length(p.raw) >= 3 AND co.is_published AND s.sc >= 0.30
       AND NOT (co.title ~* p.rp
             OR coalesce(co.description, '') ~* p.rp
             OR (p.wp IS NOT NULL AND (
                  co.title ~* ANY (p.wp)
               OR coalesce(co.description, '') ~* ANY (p.wp)
               OR EXISTS (SELECT 1 FROM course_tag_assignments a JOIN course_tags t ON t.id = a.tag_id
                           WHERE a.course_id = co.id AND t.name ~* ANY (p.wp))
               OR EXISTS (SELECT 1 FROM lessons l WHERE l.course_id = co.id
                            AND (coalesce(l.title, '') ~* ANY (p.wp)
                              OR coalesce(l.description, '') ~* ANY (p.wp)
                              OR coalesce(l.content, '') ~* ANY (p.wp))))))
  ) results
  ORDER BY results.rank ASC, results.score DESC, results.title ASC
  LIMIT 300;
$function$;

REVOKE ALL ON FUNCTION public.search_temple(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.search_temple(text) TO authenticated;