          ...
          <CardContent className="space-y-2">
            {list.map((s) => (
              <div key={s.id} className="flex items-start gap-2 border-b border-border pb-2 last:border-0">
                ...
                <Button variant="ghost" size="icon" onClick={() = aria-label="Toggle favourite"> fav.mutate(s)}>
                  <Star className={`h-4 w-4 ${s.is_favourite ? "fill-primary text-primary" : ""}`} />
                </Button>
                <Button variant="ghost" size="icon" onClick={() = aria-label="Copy to clipboard"> copy(s.text)}>
                  <Copy className="h-4 w-4" />
                </Button>
                {!s.is_seed && (
                  <Button variant="ghost" size="icon" onClick={() = aria-label="Delete"> del.mutate(s.id)}>
